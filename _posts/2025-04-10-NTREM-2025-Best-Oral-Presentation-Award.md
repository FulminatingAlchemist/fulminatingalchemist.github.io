---
layout: post
title: NTREM 2025 – Best Oral Presentation Award
date: 2025-04-10 09:00:00
description: Attending NTREM 2025 and receiving the Best Oral Presentation award.
tags:
  - conference
  - explosives
  - PhD
  - research
  - award
categories: personal-updates
thumbnail: assets/img/NTREM2025/ntrem_award.jpg
---

I recently had the honour of attending the **27th New Trends in Research of Energetic Materials (NTREM)** conference in Pardubice, Czech Republic.

During the event, I presented my research titled:

**“Novel explosive method for the synthesis of silver nanoparticles.”**

I'm delighted to share that I was awarded **1st place for Best Oral Presentation**. The conference brought together researchers from across academia and industry, and it was a pleasure to take part in such a vibrant exchange of ideas.

Congratulations to **Marcin Gerlich** as well, who received **Best Poster Presentation** for his excellent work.

<div class="row mt-3">
    <div class="col-sm mt-3 mt-md-0">
        {% include figure.liquid loading="eager" path="assets/img/NTREM2025/ntrem_award.jpg" class="img-fluid rounded z-depth-1" zoomable=true %}
    </div>
</div>
<div class="caption">
    On stage with Jiří Pachman,Marcin Gerlich and Adam Cumming during the award ceremony.
</div>

<div class="row mt-3">
    <div class="col-sm mt-3 mt-md-0">
        {% include figure.liquid loading="eager" path="assets/img/NTREM2025/ntrem_diploma.jpg" class="img-fluid rounded z-depth-1" zoomable=true %}
    </div>
</div>
<div class="caption">
    Best Oral Presentation – Diploma from NTREM 2025.
</div>

<div class="row mt-3">
    <div class="col-sm mt-3 mt-md-0">
        {% include figure.liquid loading="eager" path="assets/img/NTREM2025/ntrem_group.jpg" class="img-fluid rounded z-depth-1" zoomable=true %}
    </div>
</div>
<div class="caption">
    Group photo of NTREM 2025 attendees.
</div>
